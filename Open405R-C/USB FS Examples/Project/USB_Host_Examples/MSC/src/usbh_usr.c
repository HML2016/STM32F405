...
void USBH_USR_Init(void)
{
  ...
  STM324xG_LCD_Init();
#else
 #error "Missing define: Evaluation board (ie. USE_STM324xG_EVAL)"
#endif
...
int USBH_USR_MSC_Application(void)
{
  ...
  case USH_USR_FS_WRITEFILE:
    ...
    LCD_SetTextColor(Green);
    LCD_DisplayStringLine( LCD_PIXEL_HEIGHT - 42, "                                              ");
    LCD_DisplayStringLine( LCD_PIXEL_HEIGHT - 30, "Press USER Key to write file");
    LCD_SetTextColor(LCD_LOG_DEFAULT_COLOR); 
    ...
    LCD_SetTextColor(Green);
    LCD_DisplayStringLine( LCD_PIXEL_HEIGHT - 42, "                                              ");
    LCD_DisplayStringLine( LCD_PIXEL_HEIGHT - 30, "To start Image slide show Press USER Key.");
    LCD_SetTextColor(LCD_LOG_DEFAULT_COLOR); 
  ...
  if (res == FR_OK) {
    while(HCD_IsDeviceConnected(&USB_OTG_Core)) 
    {
      ...
      if(line_idx > 9)
      {
        ...
        LCD_SetTextColor(Green);
        LCD_DisplayStringLine( LCD_PIXEL_HEIGHT - 42, "                                              ");
        LCD_DisplayStringLine( LCD_PIXEL_HEIGHT - 30, "Press USER Key to continue...");
        LCD_SetTextColor(LCD_LOG_DEFAULT_COLOR); 
...
//   LCD_WriteRAM_Prepare(); /* Prepare to write GRAM */
  ...
  while (HCD_IsDeviceConnected(&USB_OTG_Core))
  {
    ...
    {
	  LCD_WRITE_DATA(((uint16_t)((Image_Buf[i+2] >> 3) << 11) | (uint16_t)((Image_Buf[i+1] >> 2) << 5) | (uint16_t)(Image_Buf[i] >> 3))); 
    } 
  }
	LCD_RS_L();
	LCD_CS_H();
